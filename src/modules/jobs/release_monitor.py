...
class ReleaseMonitor:
    ...
    async def _check_for_new_releases(self):
        ...
        if not tracked_list_slug:
            logger.debug("No list is being tracked for releases, skipping check.")
            return
        ...
        for repo_name in repo_names:
            try:
                ...
                if not (repo_data and repo_data.repository and repo_data.repository.latest_release and repo_data.repository.latest_release.nodes):
                    continue # No releases for this repo
                ...
                if known_tag != latest_tag:
                    logger.info(f"New release found for {repo_name}! Old: {known_tag}, New: {latest_tag}. Queueing for notification.")
                    await self.repo_queue.put(repo_name)
                
                # Always update to the latest known tag
