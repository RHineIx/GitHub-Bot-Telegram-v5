# src/rhineix_github_bot/core/logging_setup.py
...
async def log_sender_task(bot: Bot, channel_id: str):
    ...
    while True:
        try:
            ...
            logging.getLogger(__name__).error(f"FATAL: Could not send log to Telegram: {e}")
...
def setup_logging(settings: Settings) -> bool:
    ...
    if settings.log_channel_id:
        ...
        logging.info(f"TelegramLogHandler configured for channel {settings.log_channel_id}.")
        return True
