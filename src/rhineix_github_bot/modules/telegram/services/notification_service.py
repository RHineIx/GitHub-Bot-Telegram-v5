# src/rhineix_github_bot/modules/telegram/services/notification_service.py
...
from rhineix_github_bot.modules.github.formatter import RepoFormatter
from rhineix_github_bot.utils import (
...
PERMANENT_TELEGRAM_ERRORS = {
    "chat not found", "bot was kicked", "bot was blocked by the user", "user is deactivated", "chat was deleted",
}
...
class NotificationService:
    ...
    async def process_and_send(self, repo_full_name: str):
        ...
        if not repo_data or not repo_data.repository:
            logger.error(f"Could not fetch GraphQL data for {repo_full_name}. Aborting.")
            return
        ...
        if self.summarizer and await self.db_manager.are_ai_features_enabled() and readme_content:
        ...
        if not media_group:
            async with aiohttp.ClientSession() as session:
                social_image_url = await scrape_social_preview_image(owner, repo_name, session)
                if social_image_url: media_group.append(InputMediaPhoto(media=social_image_url))
    ...
    async def _build_media_group(self, urls: List[str]) -> List[InputMediaPhoto | InputMediaVideo]:
        # This method remains the same as our previous version
        media_group = []
        if not urls: return media_group
        async with aiohttp.ClientSession() as session:
            ...
            for i, result in enumerate(media_info_results):
                ...
                if "video" in content_type: media_group.append(InputMediaVideo(media=final_url))
                elif "image" in content_type: media_group.append(InputMediaPhoto(media=final_url))
        return media_group

    async def _send_notification(
        self, target_id: str, caption: str, media_group: List[InputMediaPhoto | InputMediaVideo]
    ):
        # This method also remains the same, with the self-healing logic
        chat_id, thread_id = ( (target_id.split("/")[0], int(target_id.split("/")[1])) if "/" in target_id else (target_id, None) )
        try:
            if media_group:
                media_group[0].caption = caption
                media_group[0].parse_mode = "HTML"
                if len(media_group) == 1 and isinstance(media_group[0], InputMediaPhoto):
                    await self.bot.send_photo(chat_id=chat_id, photo=media_group[0].media, caption=caption, parse_mode="HTML", message_thread_id=thread_id)
                else:
                    await self.bot.send_media_group(chat_id=chat_id, media=media_group, message_thread_id=thread_id)
            else:
                await self.bot.send_message(chat_id, caption, parse_mode="HTML", disable_web_page_preview=False, message_thread_id=thread_id)
        except TelegramAPIError as e:
            error_message = str(e).lower()
            if any(p_error in error_message for p_error in PERMANENT_TELEGRAM_ERRORS):
                logger.warning(f"Permanent error for destination {target_id}: {e}. Removing.")
